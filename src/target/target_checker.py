...
class TargetChecker(object):
    ...
    def checkState(self, is_digital=True):
        ...
        try:
            ...
            in_stock = any(x['location_available_to_promise_quantity'] > 0 or x['in_store_only']['availability_status'] not in ['OUT_OF_STOCK', 'NOT_SOLD_IN_STORE'] or x['order_pickup']['availability_status'] != 'UNAVAILABLE' for x in locations)
